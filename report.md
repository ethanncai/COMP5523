...
Captured frames flow spread by `AsyncStream<CMSampleBuffer>` → `AsyncStream<CVImageBuffer>` and use `.bufferingNewest(1)` that means the system only reserves the latest frame. This design can avoid accumulating delays during slower inference and speech playback. The preview is responsible for display and it is separate from inference process  which mainly utilize the latest frame to model inference. IOS is convenient for dynamic adjustment according to device position.

### 2.4 Continuous Guidance Loop

We design a core mechanism which is a continuous guidance loop. The system will automatically read the latest frame, build prompts and call remote inferences when users provide a voice command. Then, The system plays the generated instructions in voice form and execute next command when playback finished. The current task will be terminated when users call `stopGuidance()` that means all states will be reset.


### 2.5 Prompt Construction and Response Constraints

We design `ConcisePromptTemplate.swift` for generating the prompt that means the system will execute a series of processes including removing newline, regulating quotation mark style and cutting blank. The final prompt is responsible for assisting blind users to grasp specific drinks and out a limited and brief voice command.
